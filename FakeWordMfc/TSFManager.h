...
public:
    ...
    CTextStore(CTSFManager* pManager, HWND hwndEdit) :
        ...
        m_viewCookie(0)
    {
    std::cout << "[CTextStore::CTextStore] ctor hwndEdit=" << (void*)hwndEdit << "\n";
        m_selection.acpStart = 0;
        m_selection.acpEnd = 0;
        m_selection.style.ase = TS_AE_NONE;
        m_selection.style.fInterimChar = FALSE;
        m_compSel.acpStart = 0;
        m_compSel.acpEnd = 0;
        m_compSel.style.ase = TS_AE_NONE;
        m_compSel.style.fInterimChar = FALSE;
    }
    ...
    virtual ~CTextStore()
    {
    std::cout << "[CTextStore::~CTextStore] dtor\n";
        if (m_pSink)
        {
            m_pSink->Release();
            m_pSink = nullptr;
    ...
    {
    std::cout << "[CTextStore::Initialize] pContext=" << (void*)pContext << " clientId=" << clientId << "\n";
        m_pContext = pContext;
        m_pContext->AddRef();
        m_clientId = clientId;
        ...
        if (SUCCEEDED(hr))
        {
            if (m_CompSinkCookie != TF_INVALID_COOKIE)
            {
                pSource->UnadviseSink(m_CompSinkCookie);
                m_CompSinkCookie = TF_INVALID_COOKIE;
            }
            pSource->AdviseSink(IID_ITfContextOwnerCompositionSink,
            ...
            _com_error err(hr);
            std::cout << "m_pThreadMgr->AssociateFocus error: " << wstr2str_2ANSI(err.ErrorMessage()) << "code" << hr;
        }
    }
    ...
    {
    std::cout << "[CTextStore::QueryInterface] riid==IID_IUnknown? " << (riid == IID_IUnknown) << " riid==IID_ITextStoreACP? " << (riid == IID_ITextStoreACP) << "\n";
    ...
    std::cout << "[CTextStore::AddRef] newRef=" << ret << "\n";
    return ret;
    }
    ...
    STDMETHODIMP_(ULONG) Release() override
    {
        ...
        std::cout << "[CTextStore::Release] newRef=" << cRef << "\n";
        if (cRef == 0)
        {
            std::cout << "[CTextStore::Release] deleting self\n";
            delete this;
        }
        return cRef;
    }
    ...
    {
    std::cout << "[CTextStore::AdviseSink] riid==IID_ITextStoreACPSink? " << (riid == IID_ITextStoreACPSink) << " punk=" << punk << " dwMask=" << dwMask << "\n";
    ...
    {
    std::cout << "[CTextStore::UnadviseSink] punk=" << punk << "\n";
        if (m_pSink)
        {
            m_pSink->Release();
            m_pSink = nullptr;
    ...
    {
    std::cout << "[CTextStore::GetStatus] pdcs=" << pdcs << "\n";
        if (!pdcs)
            return E_INVALIDARG;
    ...
    {
    std::cout << "[CTextStore::QueryInsert] acpTestStart=" << acpTestStart << " acpTestEnd=" << acpTestEnd << " cch=" << cch << "\n";
    ...
    {
    std::cout << "[CTextStore::GetSelection] ulIndex=" << ulIndex << " ulCount=" << ulCount << "\n";
        // 参数验证
        ...
        *pcFetched = 1;
        std::cout << "[GetSelection] Current Selection: " << m_selection.acpStart << " -> " << m_selection.acpEnd << "\n";
        return S_OK;
    }
    /**
    ...
    {
    std::cout << "[CTextStore::SetSelection] ulCount=" << ulCount << " pSelection=" << pSelection << "\n";
        if (!pSelection)
            return E_INVALIDARG;
        //if (ulCount != 1)
        //    return E_NOTIMPL;
        ...
        std::cout << "[SetSelection] Current Selection: " << m_selection.acpStart << " -> " << m_selection.acpEnd << "\n";
        return S_OK;
    }
    ...
    {
    std::cout << "[CTextStore::GetText] acpStart=" << acpStart << " acpEnd=" << acpEnd << " cchPlainReq=" << cchPlainReq << "\n";
        // 初始化参数
        if (prgRunInfo)
        {
    ...
    std::cout << "[CTextStore::GetText] pcchPlainOut=" << *pcchPlainOut << "\n";
    ...
    {
        std::cout << "[SetText] dwFlags: " << dwFlags << "\n";
        std::cout << "[SetText] pchText: " << (pchText ? wstr2str_2ANSI(pchText) : "") << "\n";
        std::cout << "[SetText] Current Selection: " << m_selection.acpStart << " -> " << m_selection.acpEnd << "\n";
        std::cout << "[SetText] Current Selection Param: " << acpStart << " -> " << acpEnd << "\n";
        // 处理文本设置
        if (pchText && cch > 0)
        {
            // 更新文本
    ...
    {
    std::cout << "[CTextStore::GetFormattedText] acpStart=" << acpStart << " acpEnd=" << acpEnd << "\n";
        return E_NOTIMPL;
    }
    ...
    {
    std::cout << "[CTextStore::GetEmbedded] acpPos=" << acpPos << "\n";
        return E_NOTIMPL;
    }
    ...
    {
    std::cout << "[CTextStore::QueryInsertEmbedded] pguidService=" << pguidService << " pFormatEtc=" << pFormatEtc << "\n";
        if (pfInsertable)
            *pfInsertable = FALSE;
        return S_OK;
    }
    ...
    {
    std::cout << "[CTextStore::InsertEmbedded] dwFlags=" << dwFlags << " acpStart=" << acpStart << " acpEnd=" << acpEnd << " pDataObject=" << pDataObject << "\n";
        return E_NOTIMPL;
    }
    ...
    {
    std::cout << "[CTextStore::InsertTextAtSelection] dwFlags=" << dwFlags << " pacpStart=" << (void*)pacpStart << " pacpEnd=" << (void*)pacpEnd << " cch=" << cch << "\n";
    // 参数验证
        if (!pchText || cch == 0)
        {
            std::cout << "[InsertTextAtSelection] dwFlags: " << dwFlags
                << ", pchText: " << ""
                << ", *pacpStart: " << *pacpStart
                << ", *pacpEnd: " << *pacpEnd
                << "\n";
    ...
    STDMETHODIMP InsertEmbeddedAtSelection(DWORD dwFlags, IDataObject* pDataObject,
        LONG* pacpStart, LONG* pacpEnd, TS_TEXTCHANGE* pChange) override
    {
    std::cout << "[CTextStore::InsertEmbeddedAtSelection] dwFlags=" << dwFlags << " pDataObject=" << pDataObject << "\n";
        return E_NOTIMPL;
    }
    ...
    {
    std::cout << "[CTextStore::RequestSupportedAttrs] dwFlags=" << dwFlags << " cFilterAttrs=" << cFilterAttrs << "\n";
        return S_OK;
    }
    ...
    {
    std::cout << "[CTextStore::RequestAttrsAtPosition] acpPos=" << acpPos << " cFilterAttrs=" << cFilterAttrs << " dwFlags=" << dwFlags << "\n";
        return S_OK;
    }
    ...
    STDMETHODIMP RequestAttrsTransitioningAtPosition(
        ...
        /* [in] */ DWORD dwFlags)
    {
    std::cout << "[CTextStore::RequestAttrsTransitioningAtPosition] acpPos=" << acpPos << " cFilterAttrs=" << cFilterAttrs << " dwFlags=" << dwFlags << "\n";
        return E_NOTIMPL;
    }
    ...
    {
    std::cout << "[CTextStore::FindNextAttrTransition] acpStart=" << acpStart << " acpHalt=" << acpHalt << "\n";
        return E_NOTIMPL;
    }
    ...
    {
    std::cout << "[CTextStore::RetrieveRequestedAttrs] ulCount=" << ulCount << "\n";
        if (pcFetched)
            *pcFetched = 0;
        return S_OK;
    }
    ...
    {
    std::cout << "[CTextStore::GetEndACP] pacp=" << pacp << "\n";
        if (!pacp)
            return E_INVALIDARG;
    ...
    {
    std::cout << "[CTextStore::GetActiveView] pvcView=" << pvcView << "\n";
        if (!pvcView)
            return E_INVALIDARG;
    ...
    {
    std::cout << "[CTextStore::GetWnd] vcView=" << vcView << " phwnd=" << phwnd << "\n";
        if (!phwnd)
            return E_INVALIDARG;
    ...
    {
    std::cout << "[CTextStore::RequestLock] dwLockFlags=" << dwLockFlags << " currentLockType=" << m_dwLockType << "\n";
        if (!phrSession)
            return E_INVALIDARG;
    ...
    {
    std::cout << "[CTextStore::GetACPFromPoint] vcView=" << vcView << " pt=" << (pt ? ("(" + std::to_string(pt->x) + "," + std::to_string(pt->y) + ")") : std::string("null")) << " dwFlags=" << dwFlags << "\n";
        if (!pt || !pacp)
            return E_INVALIDARG;
    ...
    class TestLogRectInfo {
        RECT* prc;
    public:
        TestLogRectInfo(RECT* rc) {
            prc = rc;
        }

        ~TestLogRectInfo() {
            std::cout << "[ITextStoreACP::GetTextExt] Rect(x, y, width, height): "
                << prc->left << ", " << prc->top << ", "
    ...
    {
    std::cout << "[CTextStore::GetTextExt] vcView=" << vcView << " acpStart=" << acpStart << " acpEnd=" << acpEnd << " prc=" << prc << "\n";
        if (!prc || !pfClipped)
            return E_INVALIDARG;
        prc->left = 0;
        prc->right = 0;
    ...
    {
    std::cout << "[CTextStore::GetScreenExt] vcView=" << vcView << " prc=" << prc << "\n";
        if (!prc)
            return E_INVALIDARG;
...
public:
    ...
    {
        std::cout << "[ITfContextOwnerCompositionSink] OnStartComposition\n";
        *pfOk = TRUE;                     // 同意开始
        m_isComp = TRUE;
        return S_OK;
    }
    ...
    STDMETHODIMP OnUpdateComposition(ITfCompositionView* pView,
        ITfRange* pRangeNew)
    {
        std::cout << "[ITfContextOwnerCompositionSink] OnUpdateComposition\n";
        m_isComp = TRUE;
    ...
    {
        std::cout << "[ITfContextOwnerCompositionSink] OnEndComposition\n";
